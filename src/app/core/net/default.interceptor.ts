...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  private handleData(ev): Observable<any> {
    ...
    switch (ev.status) {
      case 200:
        if (ev.body && ev.body.code === 401) {
          this.notification.error(`登录超时，请重新登录。`, ``);
          // 清空 token 信息
      ...
      default:
        if (ev instanceof HttpErrorResponse) {
          // 强制清除 loading
          this.throwError(ev as HttpErrorResponse);
        }
        break;
    }
    return of(ev);
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    // const tempReq = req.body ? this.filterParams<HttpRequest<any>>(req) : req;
